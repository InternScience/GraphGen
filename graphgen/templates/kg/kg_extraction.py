...
This extraction is optimized for memory-system documents (e.g., DRAM, SRAM, HBM, GDDR, LPDDR, DIMM, memory controller, channel, bank, rank, timing parameters such as tRCD/tRP/tRAS, bandwidth, latency, capacity, process node, and power).
...
Note: Historical examples below may use legacy entity/relationship field counts. In real extraction output, you MUST use the 5-field entity format and 7-field relationship format defined above.
...
本任务重点适配存储器系统相关文本（如 DRAM、SRAM、HBM、GDDR、LPDDR、DIMM、内存控制器、通道、bank、rank、tRCD/tRP/tRAS、带宽、延迟、容量、工艺节点、功耗等）。
...
注意：下面的历史示例可能使用旧版实体/关系字段格式。实际抽取输出时，必须使用上面定义的5字段实体格式和7字段关系格式。
